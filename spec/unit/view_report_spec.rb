...
module EconView
  describe ViewReport do
    it "should obtain a deduped list of countries from its indicators" do
      cpi = double('cpi')
      expect(cpi).to receive(:countries).and_return([:afghanistan, :algiers, :venezuela])
      cab = double('cab')
      expect(cab).to receive(:countries).and_return([:afghanistan, :greece])
